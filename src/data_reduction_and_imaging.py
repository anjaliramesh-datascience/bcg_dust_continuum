...
def alma_imaging(file:str):
    ...
    # starting the loop to start the imaging of ALMA source with different image parameters like image size, cell size, deconvolving method, object point spread function, noise distribution, pixel weighting scheme, etc.
    for i in range(len(weights)):
        ...
        specmode = p.specmode, gridder = p.gridder, deconvolver = p.deconvolver, usemask = p.usemask, mask = mask, weighting = weights[i], 
        niter = p.niter, field = p.field, pbcor = p.pbcor, robust = p.robust, threshold = p.threshold, 
        interactive = p.interactive, uvtaper = tapers[i])
